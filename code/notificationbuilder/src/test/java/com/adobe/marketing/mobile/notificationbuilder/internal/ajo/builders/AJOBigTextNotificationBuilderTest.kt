...
class AJOBigTextNotificationBuilderTest {
    ...
    @Test
    fun `construct returns NotificationCompat Builder for full bigtext payload with fit_center icon`() {
        val pushTemplate = AJOBigTextPushTemplate(
            MapData(
                mutableMapOf(
                    PushPayloadKeys.TEMPLATE_TYPE to PushTemplateType.AJO_BIG_TEXT.value,
    ...
    @Test
    fun `construct returns NotificationCompat Builder with large icon and no collapsed text`() {
        val pushTemplate = AJOBigTextPushTemplate(
            MapData(
                mutableMapOf(
                    PushPayloadKeys.TEMPLATE_TYPE to PushTemplateType.AJO_BIG_TEXT.value,
    ...
    @Test
    fun `construct returns NotificationCompat Builder when collapsed text is absent and collapses with body text`() {
        val pushTemplate = AJOBigTextPushTemplate(
            MapData(
                mutableMapOf(
                    PushPayloadKeys.TEMPLATE_TYPE to PushTemplateType.AJO_BIG_TEXT.value,
    ...
    fun `construct uses silent channel when template is from intent`() {
        val bundle = Bundle().apply {
            ...
            putString(PushPayloadKeys.AJO_TEMPLATE_PROPERTIES, AJO_MOCKED_BIGTEXT_PROPS_FULL)
        }
    ...
    @Test
    fun `construct reuses existing notification channel`() {
